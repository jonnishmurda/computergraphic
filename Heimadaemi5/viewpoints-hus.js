    ...
    window.addEventListener("keydown", function (e) {
        switch (e.keyCode) {
            ...
            case 56:	// 8: from beside the car
                ...
                break;
            case 57:	// 9: still in progress --from beside the car
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "2: Horfa á bílinn innan úr hringnum";
                break;
...
function render() {
    ...
    switch (view) {
        //      Breytið aðeins sjónarhorni 1 þannig að notandinn getur þar notað músina til að
        //      snúa öllu líkaninu, svipað og höfum gert með mörg sýnisforritin (t.d. solkerfi).
        case 1:
            // Distant and stationary viewpoint with mouse control
            mv = lookAt(vec3(250.0, 0.0, 100.0 + height), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 1.0));
            mv = mult(mv, rotateX(spinX));   // Rotate around X-axis
            mv = mult(mv, rotateY(spinY));   // Rotate around Y-axis
            drawScenery(mv);
        ...
        case 9:
            // ofan á þaki hússins sem er staðsett í -40, 140
            mv = lookAt(vec3(-40, 140, 17.5 + height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0));
            drawScenery(mv);
            mv = mult(mv, translate(carXPos, carYPos, 0.0));
            mv = mult(mv, rotateZ(-carDirection));
            drawCar(mv);
